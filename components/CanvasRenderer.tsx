  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        <Panel position="top-left" className="bg-slate-900/95 border border-slate-800 p-2.5 rounded-2xl shadow-2xl backdrop-blur-xl max-w-lg space-y-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <span className={`text-[9px] font-mono px-2 py-0.5 rounded border flex items-center gap-1 ${
                ...
                <span className={`w-1.5 h-1.5 rounded-full ${
                  loopStatus === 'evaluating' ? 'bg-amber-400 animate-ping' : 'bg-emerald-400'
                }`} />
                <span>{loopStatus.toUpperCase()}</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              onClick={() => evaluateNow('Show where these diverge most and rank maximum disparity')}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono bg-amber-950/40 border border-amber-500/30 text-amber-300 hover:bg-amber-900/50 transition-colors disabled:opacity-50"
            >
            ...
            <button
              type="button"
              disabled={isEvaluating}
              onClick={() => evaluateNow('Show budget breakdown, cost of ownership, and price comparison')}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 hover:bg-emerald-900/50 transition-colors disabled:opacity-50"
            >
            ...
            <button
              type="button"
              disabled={isEvaluating}
              onClick={() => evaluateNow('Show milestone roadmap, implementation phases, and timeline schedule')}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono bg-indigo-950/40 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-900/50 transition-colors disabled:opacity-50"
            >
          ...
          >
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1 text-xs text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-sky-500 font-sans"
            />
            <button
              type="submit"
              disabled={isEvaluating || !tilePrompt.trim()}
              className="px-3 py-1 bg-sky-600 hover:bg-sky-500 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-lg text-xs font-medium flex items-center gap-1 transition-colors"
            >
              {isEvaluating ? (
                <>
