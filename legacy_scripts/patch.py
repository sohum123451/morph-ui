...
state_marker = "const [tableSearch, setTableSearch] = useState('');"
if 'isSidebarOpen' not in code and state_marker in code:
    sidebar_state = """const [tableSearch, setTableSearch] = useState('');
        ...
        if (Array.isArray(comp.entities) && comp.entities.length > 0) {
          resolvedEntities = comp.entities.map((e: any, idx: number) => {
            ...
            const pros = typeof e === 'object' && Array.isArray(e?.pros)
              ? e.pros.map(String).while ? e.pros.map(String).filter((p: string) => !isMissingVerdictBullet(p)) : e.pros.map(String).filter((p: string) => !isMissingVerdictBullet(p))
              : [];
            return {
              name,
  ...
  const handleNewComparison = () => {
    ...
    code = code.replace(state_marker, sidebar_state)

# 3. In handleRunComparison, update activeChatId & fetchChatHistory
if 'fetchChatHistory();' not in code:
    code = code.replace(
        'setOpenSections(!{});',
...
print('Sidebar state patched successfully')
